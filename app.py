...
@app.route('/upload')
def render_file():
    return render_template('get_img.html')

@app.route('/fileUpload', methods=['GET', 'POST'])
def upload_img():
    if request.method == 'POST':
        imgfile = request.files['file']
        imgfile.save(os.path.join('static', "test"+imgfile.filename[-4:]))
        return '업로드 성공'
...
def load_img():
    ...
    for feature in featureTypes:
        for i in range(numberOfFigs_eachType):
            ...
            #images.append(img)
            images1.append(f'{DISPLAY_FIG_PATHDIR}/staff1/{feature}/measure%23{i:03}.'+FILE_PATH[-3:])
    ...
    for feature in featureTypes:
        for i in range(numberOfFigs_eachType):
            ...
            images2.append(f'{DISPLAY_FIG_PATHDIR}/staff2/{feature}/measure%23{i:03}.'+FILE_PATH[-3:])
    # #change /test0/ to your folder of interest
